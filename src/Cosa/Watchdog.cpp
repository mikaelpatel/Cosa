...
extern "C" uint8_t Watchdog::as_prescale(uint16_t ms) {
...
extern "C" void Watchdog::begin(uint16_t ms) {
    // Map milli-seconds to watchdog prescale values
...
extern "C" void Watchdog::delay(uint32_t ms) {
    uint32_t start = Watchdog::millis();
    ms += s_ms_per_tick / 2;
    while (since(start) < ms) yield();
}

extern "C" ISR(WDT_vect) {
